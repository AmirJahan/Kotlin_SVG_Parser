...
fun PathTag.smoothCurvePiece(piece: String, curPoint: PointF, prevSegment: Segment): ArrayList<Segment> {
    ...
    for (i in 0 until points.count() step 4) {
        val smoothCurve = Segment(PathType.SmoothCurve)
        ...
        smoothCurve.cp2 = PointF(points[i].toFloat() + intCurPoint.x,
                            points[i+1].toFloat() + intCurPoint.y)

        smoothCurve.knot = PointF(points[i+2].toFloat() + intCurPoint.x,
        ...
        if (prevSegment.type == PathType.Curve || prevSegment.type == PathType.SmoothCurve) {
            ...
            smoothCurve.cp1 = PointF(xReflectionOfSecondCP, yReflectionOfSecondCP)
        } else {
            smoothCurve.cp1 = PointF(prevSegment.knot.x,
                                prevSegment.knot.y)
        }

        segments.add(smoothCurve)
    }
